    ...
    if(_s[id]._clicked)     painter.setBrush(QBrush(QColor(169,169,169)));
        ...
        if(_s[id]._side)
        {
            ...
            if(_s[id]._red)
                ...
                painter.setFont(QFont("华文行楷", _r, 700));  //字体
     ...
     if(i<32)
      {
        ...
        else if((_s[i]._side>1)&&eatRule(j,i)&&(_turn==_s[j]._red))
        {
            _s[j]._row=_s[i]._row;
            _s[j]._col=_s[i]._col;
            _s[j]._clicked=false;
...
int Board::judge1() //评判标准1.检查一方棋子是否被吃光，返回值为0，胜负暂时未分；返回值为1，红方胜利；返回值为2，黑方胜利
{
    for(int j,i=0;i<32;i++)
    {
          if(_s[i]._dead==false)
          for(j=0;j<32;j++)
              if(_s[j]._red!=_s[i]._red&&!(_s[j]._dead))    return 0;   //胜负暂时未分
          if(_s[i]._red)    return 1;   //红方胜利
          else  return 2;   //黑方胜利
    }
    return 0;
}
